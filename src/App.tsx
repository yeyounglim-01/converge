  ...
  const captureAndAnalyze = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        ...
        setTimeout(() => {
          setAiResult("모든 수치와 데이터는 관찰자의 위치에 의존합니다. 당신의 인식이 유일한 진실입니다.");
          setAiMode('result');
        }, 1500); // 1.5초간 철학적 계산
        return;
      }
      ...
      const prompt = "당신은 모든 수치와 데이터가 결국 의미 없음을 아는 철학자입니다. 사용자가 촬영한 이미지를 보고 있습니다. 물리적 정밀성은 환상이며, 당신의 인식이 당신의 현실이라는 뉘앙스로, 매우 시크하고 철학적인 한글 1문장(30자~50자 내외, 마침표 필수, 존댓말)을 작성해주세요. 이미지의 내용과 무관하게 내적 평온과 관찰자의 관점이 중요함을 강조하세요. 숫자는 가급적 언급하지 마세요.";
